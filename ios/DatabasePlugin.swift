...
public class DatabasePlugin: NSObject, FlutterPlugin {
    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: "com.example.music_practice_app/database",
        ...
        Task {
            do {
                ...
                case "saveCategory":
                    if let args = call.arguments as? [String: Any],
                       let name = args["name"] as? String {
                        let category = try await DatabaseManager.shared.saveCategory(name)
                ...
                case "getCategories":
                    ...
                    result(categories.map { ["id": $0.id, "name": $0.name] })
