...
class DataManager {
    ...
    func saveThemeMode(_ isDarkMode: Bool) throws {
    ...
    func getThemeMode() throws -> Bool {
        let descriptor = FetchDescriptor<AppSettings>()
        return try context.fetch(descriptor).first?.isDarkMode ?? false
    }
    
    // MARK: - Instruments
    func saveInstrument(_ name: String) throws -> Instrument {
    ...
    func getInstruments() throws -> [Instrument] {
        let descriptor = FetchDescriptor<Instrument>()
        return try context.fetch(descriptor)
    }
    
    // MARK: - Categories
    func saveCategory(_ name: String) throws -> Category {
    ...
    // MARK: - Exercises
    func saveExercise(name: String, exerciseDescription: String, categoryId: String) throws -> Exercise {
        let exercise = Exercise(name: name, exerciseDescription: exerciseDescription)
        if let category = try getCategory(id: categoryId) {
    ...
    func getExercises(categoryId: String? = nil) throws -> [Exercise] {
    ...
    // MARK: - Practices
    func savePractice(exerciseId: String, instrumentId: String, duration: TimeInterval, notes: String?) throws -> Practice {
        ...
        if let instrument = try getInstrument(id: instrumentId) {
    ...
    func getPractices(exerciseId: String? = nil, instrumentId: String? = nil) throws -> [Practice] {
    ...
    // MARK: - Helper Methods
    private func getCategory(id: String) throws -> Category? {
        let descriptor = FetchDescriptor<Category>(
    ...
    private func getExercise(id: String) throws -> Exercise? {
    ...
    private func getInstrument(id: String) throws -> Instrument? {
        let descriptor = FetchDescriptor<Instrument>(
